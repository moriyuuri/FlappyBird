...
class GameScene: SKScene,SKPhysicsContactDelegate /* 追加 */ {
    ...
    func setupItem() {
        ...
        // 移動する距離を計算
        let movingDistance = CGFloat(self.frame.size.width + wallSize.width*3)
        
        // 画面外まで移動するアクションを作成
        let moveItem = SKAction.moveBy(x: -movingDistance, y: 0, duration:4.5)
        ...
        let createItemAnimation = SKAction.run({
            ...
            let random_y = CGFloat.random(in: 0.4..<0.7)
            
            // 表示する位置を指定する
            item.position = CGPoint(
                x: self.frame.size.width + (wallSize.width*6) / 2,
                y: self.frame.size.height * random_y
            )
            item.zPosition = -50
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
            ...
            let soundNode = SKAudioNode(fileNamed: "sound.mp3")
